...
import { embedText, embedTexts, embedImage } from '@/lib/local-embed';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 如果指定了 itemId，只处理该条目
    if (itemId) {
      return await embedSingleItem(supabase, itemId);
    }
    ...
    for (const item of items) {
      ...
      if (item.modality === 'image') {
        ...
        try {
          ...
          // 2. 使用图片嵌入 API
          const embedding = truncateEmbedding(await embedImage(imageUrl));
  ...
  try {
    ...
    if (action === 'vectorizeTags') {
      ...
      for (const tagName of tagNames) {
        try {
          // 生成标签文本的向量
          const embedding = truncateEmbedding(await embedText(tagName));
