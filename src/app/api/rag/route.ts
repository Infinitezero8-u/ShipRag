import { llmInvoke, llmStream } from '@/lib/llm';
...
压缩后的摘要：`;
            ...
            const summaryResult = await llmInvoke([
...
请直接输出改写后的问题，不要解释：`;
        ...
        const rewriteResponse = await llmInvoke([{ role: 'user', content: rewritePrompt }], {
    ...
    // 2. 执行混合检索（实体匹配 + 语义兜底）
    const retrievalResult = await hybridSearch(rewrittenQuery, { topK: actualTopK, threshold: 0.5, modality: modality || undefined });
    const contextItems = retrievalResult.items.map(item => ({
      id: item.id,
      title: item.title,
    ...
    const searchError = retrievalResult.items.length === 0 ? new Error('no results') : null;
