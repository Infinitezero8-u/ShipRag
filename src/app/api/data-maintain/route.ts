...
import { embedText, embedTexts } from '@/lib/local-embed';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (action === 'task-detail') {
      ...
      if (task.task_type === 'port' || task.task_type === 'database') {
        // 港口数据
        const { data, error, count } = await supabase
          .from('port_data')
          .select('id, port_code, name_cn, ctry_name_cn, vector_status, embedding', { count: 'exact' })
          .order('port_code', { ascending: true })
        ...
        if (!error && data) {
          items = data.map(p => ({
            ...
            code: p.port_code,
            name: p.name_cn,
            data_type: p.ctry_name_cn,
            has_embedding: !!p.embedding
          }));
          total = count || 0;
        }
      } else if (task.task_type === 'file' || task.task_type === 'regulation') {
        // 文档数据 - 只获取该任务关联的文档
        ...
        if (targetId) {
          ...
          const { data, error } = await supabase
            .from('regulations')
            .select('id, filename, file_type, categories, file_size, storage_url, original_content')
            .eq('id', targetId)
            .single();
          ...
          const { data, error, count } = await supabase
            .from('regulations')
            .select('id, filename, file_type, categories, file_size, storage_url, original_content', { count: 'exact' })
            .order('created_at', { ascending: false })
            .range(offset, offset + pageSize - 1);
          ...
          if (!error && data) {
            items = data.map(f => ({
              ...
              content: f.original_content?.substring(0, 500)
            }));
            total = count || 0;
          }
        }
    ...
    if (action === 'process-task') {
      ...
      try {
        if (task.task_type === 'port') {
          ...
          // 调用向量化API
          const embedding = truncateEmbedding(await embedText(text));
  ...
  try {
    ...
    if (action === 'vectorize') {
      if (type === 'port') {
        ...
        try {
          ...
          // 调用embedding API
          const embedding = truncateEmbedding(await embedText(vectorText));
    ...
    if (action === 'batch-vectorize') {
      ...
      if (type === 'port') {
        ...
        // 一次性批量生成向量（大幅提升速度）
        const texts = ports.map(p =>
          `港口代码:${p.port_code} 名称:${p.name_cn} 国家:${p.ctry_name_cn} 类型:${p.port_type} 经度:${p.lon} 纬度:${p.lat} 大洲:${p.continent_name_cn}`
        );
        const embeddings = await embedTexts(texts);

        for (let i = 0; i < ports.length; i++) {
          try {
            const embedding = truncateEmbedding(embeddings[i]);
            const { error: updateError } = await supabase
              .from('port_data')
              .update({
                embedding: embedding,
              ...
              .eq('port_code', ports[i].port_code);

            if (updateError) {
              failed++;
              errors.push(`${ports[i].port_code}: ${updateError.message}`);
            } else {
              processed++;
            }
          } catch (embedError) {
            failed++;
            errors.push(`${ports[i].port_code}: ${embedError instanceof Error ? embedError.message : String(embedError)}`);
            await supabase
              .from('port_data')
              .update({ vector_status: '向量化失败', updated_at: new Date().toISOString() })
              .eq('port_code', ports[i].port_code);
    ...
    if (action === 'task-control') {
      ...
      if (operation === 'resume') {
        const { error } = await supabase
          ...
          .eq('id', taskId);
        if (error) throw error;
        return NextResponse.json({ success: true, message: '任务已继续' });
      }
      ...
      if (operation === 'start') {
        const { error } = await supabase
          ...
          .eq('id', taskId);
        if (error) throw error;
        return NextResponse.json({ success: true, message: '任务已开始' });
      }
      ...
      if (operation === 'delete') {
        ...
        if (task.task_type === 'file') {
          // 删除knowledge_items中的数据
          await supabase
            .from('knowledge_items')
            .delete()
            .eq('source', task.target_name);
          // 删除file_uploads记录
          await supabase
            .from('file_uploads')
            .delete()
            .eq('file_name', task.target_name);
        } else if (task.task_type === 'database') {
          // 数据库类型不自动删除数据，只删除任务记录
        }
    ...
    // 任务详情数据
    if (action === 'task-detail') {
      const { taskId, page = 1, pageSize = 20 } = body;
      ...
      if (task.task_type === 'database') {
        // 获取数据库表数据
        if (task.target_type === 'port') {
          const { data, error, count } = await supabase
            .from('port_data')
            .select('port_code, name_cn, ctry_name_cn, lon, lat, vector_status', { count: 'exact' })
            .order('created_at', { ascending: false })
          ...
          items = data || [];
          total = count || 0;
        } else if (task.target_type === 'route') {
          const { data, error, count } = await supabase
            .from('route_data')
            .select('orig_port, dest_port, vector_status', { count: 'exact' })
            .order('created_at', { ascending: false })
            .range((page - 1) * pageSize, page * pageSize - 1);
      ...
      } else if (task.task_type === 'file') {
        // 获取文件数据
        const { data, error, count } = await supabase
          .from('knowledge_items')
          .select('id, title, modality, source, created_at', { count: 'exact' })
          .eq('source', task.target_name)
          .order('created_at', { ascending: false })
          .range((page - 1) * pageSize, page * pageSize - 1);
