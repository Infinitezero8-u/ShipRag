...
async function parseDocumentContent(buffer: Buffer, fileType: string): Promise<string> {
  ...
  if (['pdf', 'doc', 'docx'].includes(fileType)) {
    try {
      ...
      const result = execSync(`python3 "${scriptPath}" --file "${tmpFile}"`, {
        encoding: 'utf-8',
        timeout: 60000,
      ...
      return parsed.content || `[${fileType.toUpperCase()}文档内容解析失败]`;
    } catch {
...
async function handleRevectorize(id: string, request: NextRequest) {
  ...
  if (chunkRecords.length > 0) {
    await supabase.from('regulation_chunks').insert(chunkRecords);
  }
